...
// 다마고치 상태 저장 함수
void saveTamagotchiState(const Tamagotchi::Tamagotchi* pet, const std::string& filename = "tamagotchi_save.txt") {
...
// 다마고치 상태 불러오기 함수
Tamagotchi::Tamagotchi* loadTamagotchiState(const std::string& filename = "tamagotchi_save.txt") {
    std::ifstream inFile(filename);
    if (inFile.is_open()) {
        std::string name;
        int happiness, hunger, state;

        std::getline(inFile, name);
        inFile >> happiness >> hunger >> state;

        Tamagotchi::Tamagotchi* pet = new Tamagotchi::Tamagotchi(1, name);
...
int main() {
    ...
    // 시작 시 3개의 할 일 입력받기
    std::cout << u8"프로그램을 시작하기 전에 3개의 할 일을 입력해주세요.\n" << std::endl;

    std::time_t baseTime = std::time(0);
    Plan todayPlan = planService.createPlan(baseTime, "Today");

    for (int i = 1; i <= 3; i++) {
        std::string title, description;
        int priority;

        std::cout << u8"\n" << i << u8"번째 할 일" << std::endl;
        std::cout << u8"제목: ";
        ...
        do {
            std::cout << u8"우선순위 (1:높음, 2:중간, 3:낮음): ";
            std::cin >> priority;
            std::cin.clear();
        ...
        switch (priority) {
        case 1: taskPriority = Priority::HIGH; break;
        case 2: taskPriority = Priority::MEDIUM; break;
        case 3: taskPriority = Priority::LOW; break;
        default: taskPriority = Priority::MEDIUM;
        }

        planService.addTaskToPlan(todayPlan.getPlanId(),
            TaskCreateRequest(title, description, taskPriority, baseTime + 3600, {}));
    }

    system("cls");
    std::cout << u8"할 일 3개가 등록되었습니다. 엔터 키를 누르면 시작합니다" << std::endl;
    std::cin.get();

    // 다마고치 초기화
    UserTamagotchiService tamagotchiService;
    Tamagotchi::Tamagotchi* myPet = loadTamagotchiState();
    if (!myPet) {
        myPet = new Tamagotchi::Tamagotchi(1, u8"피카츄");
        myPet->setName();
    }
            ...
            case UiState::PROMPT_ENTER_TAMAGOTCHI_MODE: {
                while (true) {
                    ...
                    if (TagamotchiInput == 'F' || TagamotchiInput == 'f') {
                        ...
                        // 다마고치 모드 종료 시 상태 저장
                        saveTamagotchiState(tamagotchiService.getTamagotchi());
                        currentAppState = UiState::SHOWING_MAIN_MENU;
                        rightPanelDynamicContent = getDefaultRightPanelContent();
                        break;
                    }
